...
def test_update_user_data_put():
    ...
    assert utils.request_put(data.BASE_URL+data.URI_USER_TO_CHANGE, data.USER_BODY) == new_data_key_list
...
def test_update_product_put():
    ...
    assert utils.request_put(data.BASE_URL + data.URI_PRODUCT_PATH, data.PRODUCT_BODY)[1:] == new_data_key_list
...
def test_update_user_data_patch():
    ...
    assert utils.request_patch(data.BASE_URL+data.URI_USER_TO_CHANGE, user_body)[0] == new_data_key
